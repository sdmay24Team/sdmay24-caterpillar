...
import { createMom } from "../graphql/mutations";
const client = generateClient();
export default function MomCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setNumEggs(initialValues.NumEggs);
    setDateFirstHatch(initialValues.DateFirstHatch);
    setTotalHatched(initialValues.TotalHatched);
    setErrors({});
  };
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createMom.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "MomCreateForm")}
      {...rest}
    >
      <TextField
        label="Num eggs"
